...
- Run a focused test with `just test <path-or-node-id>`; for backend behavior changes or release preparation, run `just test` and the relevant project checks. Documentation-only edits do not require a database suite.
- Verification recipes must disable inherited debug and dev-autologin flags without loading owner secrets. Test tooling by running its public command against controlled inputs, not by asserting source text.
